...
def preprocess_wav_torch(wav, source_sr=None):
    """
    Applies the preprocessing operations used in training the Speaker Encoder to a waveform
    either on disk or in memory. The waveform will be resampled to match the data hyperparameters.
...
def wav_to_mel_spectrogram_torch_preprocess(wav):
    """
    Derives a mel spectrogram ready to be used by the encoder from a preprocessed audio waveform.
    Note: this not a log-mel spectrogram.
    wav: Tensor (T)
...
if __name__ == '__main__':
    one = torch.ones((12800))
    two = np.ones(12800)
    one_out = wav_to_mel_spectrogram_torch(one)
    two_out = wav_to_mel_spectrogram(two)
    print(one_out.shape, two_out.shape)
